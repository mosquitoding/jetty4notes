...
public class SslConnection extends AbstractConnection
{
    ...
    public void onOpen()
    {
        super.onOpen();//AbstractConnection#onOpen
    ...
    public class DecryptedEndPoint extends AbstractEndPoint
    {
        ...
        public boolean flush(ByteBuffer... appOuts) throws IOException
        {
            ...
            //需要明确几个概念  appouts  flushed  filled
            if (LOG.isDebugEnabled())
            {
            ...
            try
            {
                synchronized (this)
                {
                    try
                    {
                        if (_cannotAcceptMoreAppDataToFlush)//不能接收更多的app的需要flush出去的数据
                        {
                            if (_sslEngine.isOutboundDone())//如果sslEngine的outbound已经done了
                        ...
                        while (true)
                        {
                            ...
                            int pos = BufferUtil.flipToFill(_encryptedOutput);//指示从哪里开始可以可入
                            SSLEngineResult wrapResult;
                            try
                            {
                                wrapResult = _sslEngine.wrap(appOuts, _encryptedOutput);//加密数据appOuts，加密数据保存在_encryptedOutput
                            }
                            finally
                            {
        ...
        protected void doShutdownInput()
        {
            try
            {
                 synchronized (_decryptedEndPoint)
                 {
                     ...
                     {
                         if (LOG.isDebugEnabled())
                             LOG.debug("doShutdownInput: ishut={} sslConnection {}",ishut, SslConnection.this);
                         _sslEngine.closeInbound();
                         LOG.debug("doShutdownInput: isInboudDone {} sslConnection {}",_sslEngine.isInboundDone(), SslConnection.this);
                         getEndPoint().shutdownOutput();
                     }
                 }
            }
        ...
        public void doShutdownOutput()
        {
            try
            {
                ...
                if (close)
                    getEndPoint().close();//此处关闭的EngPoint是？
                else
                    ensureFillInterested();
            }
            catch (Throwable x)
        ...
        public void doClose()
        {
            ...
            LOG.debug("this {} isInputShutdown {} isOutputShutdown {} getEndPoint {}",
                    this,isInputShutdown(),isOutputShutdown(),getEndPoint());
            doShutdownInput();
            doShutdownOutput();//case1：关闭sslEngine的outbound  case2:由于isInputShutdown和isOutputShutdown都是true，因此此时实际什么也不做
            getEndPoint().close();//SocketChannelEndPoint
            super.doClose();
        }
