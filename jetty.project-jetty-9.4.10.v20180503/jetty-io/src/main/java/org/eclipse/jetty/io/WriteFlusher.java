...
abstract public class WriteFlusher
{
    ...
    {
        State current = _state.get();
        while (current.getType() == StateType.FAILED)//完全是死循环啊
        {
    ...
    public void write(Callback callback, ByteBuffer... buffers) throws WritePendingException
    {
        ...
        if (!updateState(__IDLE, __WRITING))//状态必须是__IDLE转化为__WRITING
        ...
        try
        {
            ...
            if (buffers != null)
            {
                ...
                if (updateState(__WRITING, pending))//有没有可能多个线程并发处理？
                    onIncompleteFlush();
                else
                    fail(pending);
                return;
    ...
    public void completeWrite()
    {
        ...
        PendingState pending = (PendingState)previous;//可以确认之前的状态就是PengingState
        if (!updateState(pending, __COMPLETING))//从pending状态切换到__COMPLETING状态
            return; // failure already handled.
        ...
        try
        {
            ...
            buffers = flush(buffers);//从pengding中获取buffers，flush出去
            ...
            if (buffers != null)
            {
                ...
                if (buffers != pending.getBuffers())//如果数据有变化，则需要重新封装pengding状态
    ...
    protected ByteBuffer[] flush(ByteBuffer[] buffers) throws IOException
    {
        ...
        while (progress && buffers != null)
        {
            ...
            boolean flushed = _endPoint.flush(buffers);// https  h2 是不是有所不同?
            long after = remaining(buffers);
            long written = before - after;//flush了多少数据
            ...
            if (flushed)// flushed为true，说明数据全部flush完毕
                return null;
            ...
            if (index > 0)//有可能第一个buffer的remaing就没有flush完毕，因此index可能为0？
